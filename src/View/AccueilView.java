...
public class AccueilView extends JPanel {
	...
	public AccueilView(JFrame f) throws IOException {
		this.frameglobal = f;
		acc = this;
		frameglobal.setPreferredSize(new Dimension(1200, 1200));
		// for unix
	...
	public void accUpdate() {
		...
		for (int i = 0; i < maxfile; i++) {
			...
			if (i < listlevelName.size()) {
				CreateLevelButton(listlevelName.get(i), listlevelName.size(), true);
			} else {
				this.add(new JLabel());
			}
			if (i < listlevelName2.size()) {
				CreateLevelButton(listlevelName2.get(i), listlevelName2.size(), false);
			} else {
				this.add(new JLabel());
			}
		}
		...
		ActionListener al = new ActionListener() {
			...
			public void actionPerformed(ActionEvent e) {
				...
				try {
					...
					if (editable) {
						new GrilleView(new Model(url + bs.getName() + ".p", bs.getName()), frameglobal, acc);
					} else {
						new GrilleView(new Model(url_non_edit + bs.getName() + ".p", bs.getName()), frameglobal, acc);
					}
