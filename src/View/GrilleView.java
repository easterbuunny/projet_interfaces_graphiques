...
public class GrilleView extends JPanel {
	...
	private BufferedImage listenerRes = new BufferedImage(getSizeOfImage(), getSizeOfImage(),
			BufferedImage.TYPE_INT_ARGB);
	private Graphics g = getListenerRes().createGraphics();
	private BufferedImage returnRes = new BufferedImage(getSizeOfImage(), getSizeOfImage(),
			BufferedImage.TYPE_INT_ARGB);
		...
		for (int i = 0; i < getH(); i++) {// pour chaque ligne
			for (int j = 0; j < getW(); j++) {// pour chaque col
				...
				if (s.equals("X")) {
					// CORNER
					if ((i == 0 && j == 0)) {
						getSprites()[i][j] = corner;
						/* sprites[i][j].setImage(corner); */} else if (i == 0 && j == getW() - 1) {
						getSprites()[i][j] = rotateImage(corner, getRightAngle());
					} else if (i == getH() - 1 && j == 0) {
						getSprites()[i][j] = rotateImage(corner, getRightAngle() * 3);
					} else if (i == getH() - 1 && j == getW() - 1) {
						getSprites()[i][j] = rotateImage(corner, getRightAngle() * 2);
					}

					// BAR
					else if (i == 0 && j == 1) {
						getSprites()[i][j] = mixScore(rotateImage(bar, 0), gl.getAffichage());
					} else if (i == 0) {
						getSprites()[i][j] = rotateImage(bar, 0);
					} else if (i == getH() - 1) {
						getSprites()[i][j] = rotateImage(bar, getRightAngle() * 2);
					} else if (j == 0) {
						getSprites()[i][j] = rotateImage(bar, getRightAngle() * 3);
					} else if (j == getW() - 1) {
						getSprites()[i][j] = rotateImage(bar, getRightAngle());
					}
					...
					getSprites()[i][j] = tuile2use;
				}

				// a cell locked, can't put tuyaux : level diff
				else if (s.equals("*.")) {
					getSprites()[i][j] = tuileEmpty;
				}
				...
				// for source
					...
					// display border
					if ((i == 0 && j == 0)) {
						tuile2use = rotateImage(corner, 0);
					} else if (i == 0 && j == getW() - 1) {
						tuile2use = rotateImage(corner, getRightAngle());
					} else if (i == getH() - 1 && j == 0) {
						tuile2use = rotateImage(corner, getRightAngle() * 3);
					} else if (i == getH() - 1 && j == getW() - 1) {
						tuile2use = rotateImage(corner, getRightAngle() * 2);
					} else if (i == 0 && j == 1) {
						tuile2use = mixScore(rotateImage(bar, 0), gl.getAffichage());
					} else if (i == 0) {
						tuile2use = rotateImage(bar, 0);
					} else if (i == getH() - 1) {
						tuile2use = rotateImage(bar, getRightAngle() * 2);
					} else if (j == 0) {
						tuile2use = rotateImage(bar, getRightAngle() * 3);
					} else if (j == getW() - 1) {
						tuile2use = rotateImage(bar, getRightAngle());
					}
	...
	public void setMenu(JPanel acc) {
		...
		if (gl.getAffichage().endsWith("_NE")) {
		...
		srcRed.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (getModel().getMainGrille().getCase(0, getW() - 1).equals("X")) {
					...
					getModel().getMainGrille().setSchema(0, getW() - 1,
							new Tuyaux(Type.S, Orientation.S, Couleur.R, subImages));
		...
		srcGreen.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (getModel().getMainGrille().getCase(0, getW() - 1).equals("X")) {
					...
					getModel().getMainGrille().setSchema(0, getW() - 1,
							new Tuyaux(Type.S, Orientation.S, Couleur.G, subImages));
					getModel().updateColor(getEditor());
					getModel().getMainGrille().displayGrille();
					getModel().getMainGrille().displaySchema();
					ImageOfCell(false);
		...
		srcBlue.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (getModel().getMainGrille().getCase(0, getW() - 1).equals("X")) {
					...
					getModel().getMainGrille().setSchema(0, getW() - 1,
							new Tuyaux(Type.S, Orientation.S, Couleur.B, subImages));
					getModel().updateColor(getEditor());
					getModel().getMainGrille().displayGrille();
					getModel().getMainGrille().displaySchema();
					ImageOfCell(false);
		...
		srcYellow.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (getModel().getMainGrille().getCase(0, getW() - 1).equals("X")) {
					...
					getModel().getMainGrille().setSchema(0, getW() - 1,
							new Tuyaux(Type.S, Orientation.S, Couleur.Y, subImages));
					getModel().updateColor(getEditor());
					getModel().getMainGrille().displayGrille();
					getModel().getMainGrille().displaySchema();
					ImageOfCell(false);
		...
		visser.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if (getModel().getMainGrille().getCase(0, getW() - 1).equals("X")) {
					...
					getModel().getMainGrille().setSchema(0, getW() - 1,
							new Tuyaux(Type.V, Orientation.N, Couleur.B, subImages));
					getModel().updateColor(getEditor());
					getModel().getMainGrille().displayGrille();
					getModel().getMainGrille().displaySchema();
					ImageOfCell(false);
	...
	// Getters /Setters
	public JFrame getFrameG() {
		return frameglobal;
	}

	public AccueilView getAcc() {
		return acc;
	}
	...
	public void setEditorTrue() {
		editor = true;
	}

	public void setEditorFalse() {
		editor = false;
	}
	...
	public Tuyaux[][] getTTuyaux() {
		return tTuyaux;
	}

	public File getNewLevel() {
		return newLevel;
	}

	public void setNewLevel(File f) {
		newLevel = f;
	}

	public BufferedImage getTuile2() {
		return tuile2;
	}

	public void setTuile2(BufferedImage bi) {
		tuile2 = bi;
	}

	public ExtractSubImage getSubImages() {
		return subImages;
	}
	...
	public BufferedImage getListenerRes() {
		return listenerRes;
	}

	public void setListenerRes(BufferedImage listenerRes) {
		this.listenerRes = listenerRes;
	}

	public BufferedImage[][] getListTuyaux() {
		return listTuyaux;
	}

	public BufferedImage getReturnRes() {
		return returnRes;
	}

	public void setReturnRes(BufferedImage returnRes) {
	...
	public int[][] getCountTuyaux() {
		return countTuyaux;
	}

	public void setCountTuyaux(int countTuyaux, int i, int j) {
		this.countTuyaux[i][j] = countTuyaux;
	}
	...
	public void setSprites(BufferedImage sprites, int i, int j) {
		this.sprites[i][j] = sprites;
	}

	public void setBar2(BufferedImage bufferedImage) {
		bar2 = bufferedImage;
	}
